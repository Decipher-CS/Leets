...
class Solution:
    def maxSubArray(self, nums: List[int]) -> int:
        self.current_greatest_value = nums[0]
        self.greatest_values = {}
        def permutate_nums(nums):
            if self.greatest_values.get(f"{nums}"):
                return 0
            array_sum = sum(nums)
            self.greatest_values[f"{nums}"] = array_sum
            
            if array_sum > self.current_greatest_value:
                self.current_greatest_value = array_sum

            if len(nums) <= 1:
                return nums[0]
            else:
                permutate_nums(nums[:-1]) 
                permutate_nums(nums[1:])
                
        permutate_nums(nums)
        return self.current_greatest_value
